...
import { fetchProjectBlockWidgets, ICovalentConfig } from './global/index';
import { containerStyle, covalentStyle } from './index.css';
...
@customModule
@customElements('i-scom-covalent')
export default class ScomCovalent extends Module {
  private vStackCovalent: VStack;
  private mdConfig: Modal;
  private pnlConfig: Panel;
  private _data: ICovalentConfig = { componentId: 0 };
  tag: any = {};

  static async create(options?: ScomCovalentElement, parent?: Container) {
  ...
  private async setData(data: ICovalentConfig) {
    this._data = data;
  ...
  private async renderPlaceholder() {
    ...
    this.vStackCovalent.appendChild(
      <i-vstack
        gap={20}
        ...
        class="pointer"
      >
        <i-label caption="Covalent Block" font={{ size: '20px' }} />
        <i-icon
          name="plus"
  ...
  async init() {
    ...
    super.init();
    this.classList.add(covalentStyle);
    this.width = this.tag.width || 700;
    this.maxWidth = '100%';
